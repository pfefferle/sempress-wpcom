 ...
 * @since sempress 1.0
 */
function sempress_enhanced_image_navigation( $url, $id ) {
...
add_filter( 'attachment_link', 'sempress_enhanced_image_navigation', 10, 2 );
...
function sempress_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'sempress' ), max( $paged, $page ) );
